...
/// @notice Base abstract class with common functions for all upgradeable token pools.
...
abstract contract TokenPoolMutable is
    Initializable,
    OwnableUpgradeable,
    ...
    IERC165Upgradeable
{
    ...
    struct RemoteChainConfig {
        ...
        bytes remoteTokenAddress; // Address of the remote token, ABI encoded in the case of a remote EVM chain.
        EnumerableSetUpgradeable.Bytes32Set remotePools; // Set of remote pool hashes, ABI encoded in the case of a remote EVM chain.
    }
    ...
    /// @dev The flag that indicates if the pool is access-controlled.
    bool internal allowlistEnabled;
    /// @dev A set of addresses allowed to trigger lockOrBurn as original senders.
    /// Only takes effect if allowlistEnabled is true.
    /// This can be used to ensure only token-issuer specified addresses can move tokens.
    EnumerableSetUpgradeable.AddressSet internal allowlist;
    /// @dev The address of the router
    IRouter internal router;
    /// @dev A set of allowed chain selectors. We want the allowlist to be enumerable to
    /// be able to quickly determine (without parsing logs) who can access the pool.
    /// @dev The chain selectors are in uint256 format because of the EnumerableSet implementation.
    EnumerableSetUpgradeable.UintSet internal remoteChainSelectors;
    mapping(uint64 => RemoteChainConfig) internal remoteChainConfigs;
    ...
    mapping(bytes32 => bytes) internal remotePoolAddresses;
    ...
    function _initializeTokenPool(
        ...
        allowlistEnabled = _allowlist.length > 0;
        if (allowlistEnabled) {
            _applyAllowListUpdates(new address[](0), _allowlist);
        }
    }
    ...
    function getToken() public view returns (IERC20Upgradeable) {
        return token;
    }

    /// @notice Get RMN proxy address
    /// @return rmnProxy Address of RMN proxy
    function getRmnProxy() public view returns (address) {
        return rmnProxy;
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public pure virtual override(IERC165, IERC165Upgradeable) returns (bool) {
        return
    ...
    function getTokenDecimals() public view virtual returns (uint8) {
    ...
    {
        return abi.encode(tokenDecimals);
    }
    ...
    function _calculateLocalAmount(
        ...
        if (remoteDecimals == tokenDecimals) {
            return remoteAmount;
        }
        if (remoteDecimals > tokenDecimals) {
            uint8 decimalsDiff = remoteDecimals - tokenDecimals;
            if (decimalsDiff > 77) {
        ...
        // would overflow.
        uint8 diffDecimals = tokenDecimals - remoteDecimals;
        if (
            diffDecimals > 77 ||
    ...
    function getRemotePools(
        ...
        bytes32[] memory remotePoolHashes = remoteChainConfigs[
            remoteChainSelector
        ].remotePools.values();

        bytes[] memory remotePools = new bytes[](remotePoolHashes.length);
        for (uint256 i = 0; i < remotePoolHashes.length; ++i) {
            remotePools[i] = remotePoolAddresses[remotePoolHashes[i]];
        }
    ...
    function isRemotePool(
        ...
        return
            remoteChainConfigs[remoteChainSelector].remotePools.contains(
    ...
    function getRemoteToken(
        ...
        return remoteChainConfigs[remoteChainSelector].remoteTokenAddress;
    }
    ...
    function removeRemotePool(
        ...
        if (
            !remoteChainConfigs[remoteChainSelector].remotePools.remove(
                keccak256(remotePoolAddress)
            )
        ) {
    ...
    function isSupportedChain(
        ...
        return remoteChainSelectors.contains(remoteChainSelector);
    }

    /// @notice Get list of allowed chains
    /// @return list of chains.
    function getSupportedChains() public view returns (uint64[] memory) {
        uint256[] memory uint256ChainSelectors = remoteChainSelectors.values();
    ...
    function applyChainUpdates(
        ...
        for (uint256 i = 0; i < remoteChainSelectorsToRemove.length; ++i) {
            ...
            if (!remoteChainSelectors.remove(remoteChainSelectorToRemove)) {
        ...
        for (uint256 i = 0; i < chainsToAdd.length; ++i) {
            ...
            if (!remoteChainSelectors.add(newChain.remoteChainSelector)) {
                revert ChainAlreadyExists(newChain.remoteChainSelector);
            }

            RemoteChainConfig storage remoteChainConfig = remoteChainConfigs[
            ...
            remoteChainConfig.outboundRateLimiterConfig = RateLimiter
                .TokenBucket({
                    ...
                    lastUpdated: uint32(block.timestamp), // solhint-disable-line not-rely-on-time
    ...
    /// @dev Only callable by the owner.
    /// @param _rateLimitAdmin The new rate limiter admin address.
    function setRateLimitAdmin(address _rateLimitAdmin) external onlyOwner {
        // Allow setting to zero address to disable rate limiting
        rateLimitAdmin = _rateLimitAdmin;
        emit RateLimitAdminSet(_rateLimitAdmin);
    }
    ...
    function _consumeOutboundRateLimit(
        ...
        remoteChainConfigs[remoteChainSelector]
            .outboundRateLimiterConfig
            ._consume(amount, address(token));
    }
    ...
    /// @param chainSelectors The remote chain selector for which the rate limits apply.
    ...
    function setChainRateLimiterConfigs(
        ...
        if (msg.sender != rateLimitAdmin && msg.sender != owner())
            revert Unauthorized(msg.sender);
        if (
            chainSelectors.length != outboundConfigs.length ||
            chainSelectors.length != inboundConfigs.length
        ) {
    ...
    function _onlyOnRamp(uint64 remoteChainSelector) internal view {
        ...
        if (!(msg.sender == router.getOnRamp(remoteChainSelector)))
    ...
    function _onlyOffRamp(uint64 remoteChainSelector) internal view {
        if (!isSupportedChain(remoteChainSelector))
            revert ChainNotAllowed(remoteChainSelector);
        if (!router.isOffRamp(remoteChainSelector, msg.sender))
            revert CallerIsNotARampOnRouter(msg.sender);
    }
    ...
    function _applyAllowListUpdates(
        ...
        if (!allowlistEnabled) revert AllowListNotEnabled();

        for (uint256 i = 0; i < removes.length; ++i) {
            address toRemove = removes[i];
            if (allowlist.remove(toRemove)) {
