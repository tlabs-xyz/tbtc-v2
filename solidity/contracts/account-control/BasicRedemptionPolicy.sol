...
contract BasicRedemptionPolicy is IRedemptionPolicy, AccessControl {
    enum BulkAction { FULFILL, DEFAULT }
    ...
    function validateRedemptionRequest(
        ...
        if (qcStatus != QCData.QCStatus.Active && qcStatus != QCData.QCStatus.UnderReview) {
            return false;
        }
    ...
    function requestRedemption(
        ...
        if (redemptionId == bytes32(0)) {
            _logError(ERR_BRP_REQUEST_INVALID_ID, "requestRedemption", msg.sender, 
                     bytes32(0), "requestRedemption: Invalid redemption ID - cannot be zero bytes32");
            require(false, "BRP-1201: requestRedemption failed - Invalid redemption ID (zero bytes32)");
        }
        
        if (requestedRedemptions[redemptionId]) {
            _logError(ERR_BRP_REQUEST_ID_COLLISION, "requestRedemption", msg.sender,
                     redemptionId, "requestRedemption: Redemption ID collision - ID already used, potential attack");
            require(false, "BRP-1202: requestRedemption failed - Redemption ID already used, potential collision attack");
        }
        
        if (qc == address(0)) {
            _logError(ERR_BRP_REQUEST_INVALID_QC, "requestRedemption", msg.sender,
                     redemptionId, "requestRedemption: Invalid QC address - cannot be zero address");
            require(false, "BRP-1203: requestRedemption failed - Invalid QC address (zero address)");
        }
        
        if (user == address(0)) {
            _logError(ERR_BRP_REQUEST_INVALID_USER, "requestRedemption", msg.sender,
                     redemptionId, "requestRedemption: Invalid user address - cannot be zero address");
            require(false, "BRP-1204: requestRedemption failed - Invalid user address (zero address)");
        }
        
        if (amount == 0) {
            _logError(ERR_BRP_REQUEST_INVALID_AMOUNT, "requestRedemption", msg.sender,
                     redemptionId, string(abi.encodePacked("requestRedemption: Invalid amount (", _uintToString(amount), ") - must be greater than zero")));
            require(false, "BRP-1205: requestRedemption failed - Amount must be greater than zero");
        }
        
        if (bytes(btcAddress).length == 0) {
            _logError(ERR_BRP_REQUEST_INVALID_BTC_ADDR, "requestRedemption", msg.sender,
                     redemptionId, "requestRedemption: Invalid Bitcoin address - cannot be empty string");
            require(false, "BRP-1206: requestRedemption failed - Invalid Bitcoin address (empty string)");
        }
        
        // Business logic validation - check if redemption is allowed
        if (!validateRedemptionRequest(user, qc, amount)) {
            _logError(ERR_BRP_REQUEST_VALIDATION_FAILED, "requestRedemption", msg.sender,
                     redemptionId, string(abi.encodePacked("requestRedemption: Validation failed - user: ", _addressToString(user), 
                           ", qc: ", _addressToString(qc), ", amount: ", _uintToString(amount))));
            require(false, "BRP-1207: requestRedemption failed - Validation failed, check QC status and user eligibility");
        }
        ...
        emit RedemptionRequested(redemptionId, qc, user, amount, btcAddress, msg.sender, block.timestamp);
    ...
    function recordFulfillment(
        ...
        if (redemptionId == bytes32(0)) {
            _logError(ERR_BRP_FULFILL_INVALID_ID, "recordFulfillment", msg.sender,
                     bytes32(0), "recordFulfillment: Invalid redemption ID - cannot be zero bytes32");
            require(false, "BRP-1301: recordFulfillment failed - Invalid redemption ID (zero bytes32)");
        }
        
        if (!requestedRedemptions[redemptionId]) {
            _logError(ERR_BRP_FULFILL_NOT_REQUESTED, "recordFulfillment", msg.sender,
                     redemptionId, "recordFulfillment: Redemption not requested - ID not found in system");
            require(false, "BRP-1302: recordFulfillment failed - Redemption not requested, verify redemption ID");
        }
        
        if (bytes(userBtcAddress).length == 0) {
            _logError(ERR_BRP_FULFILL_INVALID_BTC_ADDR, "recordFulfillment", msg.sender,
                     redemptionId, "recordFulfillment: Invalid Bitcoin address - cannot be empty string");
            require(false, "BRP-1303: recordFulfillment failed - Invalid Bitcoin address (empty string)");
        }
        
        if (expectedAmount == 0) {
            _logError(ERR_BRP_FULFILL_INVALID_AMOUNT, "recordFulfillment", msg.sender,
                     redemptionId, string(abi.encodePacked("recordFulfillment: Invalid amount (", _uintToString(expectedAmount), ") - must be greater than zero")));
            require(false, "BRP-1304: recordFulfillment failed - Invalid amount (must be greater than zero)");
        }
        
        // State validation - prevent double processing with clear explanations
        if (fulfilledRedemptions[redemptionId]) {
            _logError(ERR_BRP_FULFILL_ALREADY_FULFILLED, "recordFulfillment", msg.sender,
                     redemptionId, "recordFulfillment: Already fulfilled - redemption has been processed");
            require(false, "BRP-1305: recordFulfillment failed - Redemption already fulfilled, cannot process twice");
        }
        
        if (defaultedRedemptions[redemptionId] != bytes32(0)) {
            _logError(ERR_BRP_FULFILL_ALREADY_DEFAULTED, "recordFulfillment", msg.sender,
                     redemptionId, "recordFulfillment: Already defaulted - redemption marked as failed");
            require(false, "BRP-1306: recordFulfillment failed - Redemption already defaulted, cannot fulfill");
        }
        
        // System state validation with context
        SystemState systemState = SystemState(protocolRegistry.getService(SYSTEM_STATE_KEY));
        if (systemState.isRedemptionPaused()) {
            _logError(ERR_BRP_FULFILL_SYSTEM_PAUSED, "recordFulfillment", msg.sender,
                     redemptionId, "recordFulfillment: System paused - redemptions temporarily disabled");
            require(false, "BRP-1307: recordFulfillment failed - Redemptions are paused by system administrator");
        }
        ...
        if (!_verifySPVProof(redemptionId, userBtcAddress, expectedAmount, txInfo, proof)) {
            _logError(ERR_BRP_FULFILL_SPV_FAILED, "recordFulfillment", msg.sender,
                     redemptionId, string(abi.encodePacked("recordFulfillment: SPV verification failed - address: ", userBtcAddress, 
                           ", amount: ", _uintToString(expectedAmount))));
            require(false, "BRP-1308: recordFulfillment failed - SPV proof verification failed, verify transaction data");
        }
    ...
    function flagDefault(
        ...
        if (redemptionId == bytes32(0)) {
            _logError(ERR_BRP_DEFAULT_INVALID_ID, "flagDefault", msg.sender,
                     bytes32(0), "flagDefault: Invalid redemption ID - cannot be zero bytes32");
            require(false, "BRP-1401: flagDefault failed - Invalid redemption ID (zero bytes32)");
        }
        
        if (!requestedRedemptions[redemptionId]) {
            _logError(ERR_BRP_DEFAULT_NOT_REQUESTED, "flagDefault", msg.sender,
                     redemptionId, "flagDefault: Redemption not requested - ID not found in system");
            require(false, "BRP-1402: flagDefault failed - Redemption not requested, verify redemption ID");
        }
        
        if (reason == bytes32(0)) {
            _logError(ERR_BRP_DEFAULT_INVALID_REASON, "flagDefault", msg.sender,
                     redemptionId, "flagDefault: Invalid reason - cannot be zero bytes32, audit trail required");
            require(false, "BRP-1403: flagDefault failed - Reason required for audit trail (cannot be zero bytes32)");
        }
        
        // State validation - prevent double processing with clear explanations
        if (fulfilledRedemptions[redemptionId]) {
            _logError(ERR_BRP_DEFAULT_ALREADY_FULFILLED, "flagDefault", msg.sender,
                     redemptionId, "flagDefault: Already fulfilled - cannot default completed redemption");
            require(false, "BRP-1404: flagDefault failed - Redemption already fulfilled, cannot mark as defaulted");
        }
        
        if (defaultedRedemptions[redemptionId] != bytes32(0)) {
            _logError(ERR_BRP_DEFAULT_ALREADY_DEFAULTED, "flagDefault", msg.sender,
                     redemptionId, "flagDefault: Already defaulted - redemption already marked as failed");
            require(false, "BRP-1405: flagDefault failed - Redemption already defaulted, cannot process twice");
        }
        ...
        emit RedemptionDefaultedByPolicy(redemptionId, reason, msg.sender, block.timestamp);
    ...
    function getRedemptionTimeout() external view override returns (uint256 timeout) {
        SystemState systemState = SystemState(protocolRegistry.getService(SYSTEM_STATE_KEY));
    ...
    function _verifySPVProof(
        ...
        try protocolRegistry.getService(SPV_VALIDATOR_KEY) returns (address validatorAddress) {
            ...
            return spvValidator.verifyRedemptionFulfillment(
                redemptionId,
                userBtcAddress,
                expectedAmount,
                txInfo,
    ...
    function bulkHandleRedemptions(
        ...
        if (redemptionIds.length == 0) {
            _logError(ERR_BRP_BULK_NO_REDEMPTIONS, "bulkHandleRedemptions", msg.sender,
                     bytes32(0), "bulkHandleRedemptions: No redemptions provided - array cannot be empty");
            require(false, "BRP-1501: bulkHandleRedemptions failed - No redemptions provided in array");
        }
        
        if (action == BulkAction.DEFAULT && reason == bytes32(0)) {
            _logError(ERR_BRP_BULK_REASON_REQUIRED, "bulkHandleRedemptions", msg.sender,
                     bytes32(uint256(redemptionIds.length)), 
                     string(abi.encodePacked("bulkHandleRedemptions: Reason required for DEFAULT action on ", 
                           _uintToString(redemptionIds.length), " redemptions")));
            require(false, "BRP-1502: bulkHandleRedemptions failed - Reason required for DEFAULT action (cannot be zero bytes32)");
        }
        
        for (uint256 i = 0; i < redemptionIds.length; i++) {
            bytes32 redemptionId = redemptionIds[i];
            
            if (fulfilledRedemptions[redemptionId] || defaultedRedemptions[redemptionId] != bytes32(0)) {
