...
contract ErrorCodes {
    ...
    function getErrorMessage(uint16 errorCode)
        external
        pure
        returns (string memory message)
    {
        // Protocol Registry errors
        if (errorCode == ERR_REGISTRY_INVALID_SERVICE_ADDR)
            return "ProtocolRegistry: Invalid service address";
        if (errorCode == ERR_REGISTRY_SERVICE_NOT_REGISTERED)
            return "ProtocolRegistry: Service not registered";

        // QC Data errors
        if (errorCode == ERR_QCDATA_INVALID_MANAGER)
            return "QCData: Invalid manager address";
        if (errorCode == ERR_QCDATA_INVALID_QC_ADDR)
            return "QCData: Invalid QC address";
        if (errorCode == ERR_QCDATA_QC_ALREADY_REGISTERED)
            return "QCData: QC already registered";
        if (errorCode == ERR_QCDATA_INVALID_CAPACITY)
            return "QCData: Invalid minting capacity";
        if (errorCode == ERR_QCDATA_QC_NOT_REGISTERED)
            return "QCData: QC not registered";
        if (errorCode == ERR_QCDATA_INVALID_WALLET_ADDR)
            return "QCData: Invalid wallet address";
        if (errorCode == ERR_QCDATA_WALLET_ALREADY_REGISTERED)
            return "QCData: Wallet already registered";
        if (errorCode == ERR_QCDATA_WALLET_NOT_REGISTERED)
            return "QCData: Wallet not registered";
        if (errorCode == ERR_QCDATA_WALLET_NOT_ACTIVE)
            return "QCData: Wallet not active";

        // System State errors
        if (errorCode == ERR_SYSSTATE_MINTING_ALREADY_PAUSED)
            return "SystemState: Minting already paused";
        if (errorCode == ERR_SYSSTATE_MINTING_NOT_PAUSED)
            return "SystemState: Minting not paused";
        if (errorCode == ERR_SYSSTATE_REDEMPTION_ALREADY_PAUSED)
            return "SystemState: Redemption already paused";
        if (errorCode == ERR_SYSSTATE_REDEMPTION_NOT_PAUSED)
            return "SystemState: Redemption not paused";
        if (errorCode == ERR_SYSSTATE_INVALID_THRESHOLD)
            return "SystemState: Invalid threshold value";
        if (errorCode == ERR_SYSSTATE_INVALID_TIMEOUT)
            return "SystemState: Invalid timeout value";
        if (errorCode == ERR_SYSSTATE_INVALID_COUNCIL_ADDR)
            return "SystemState: Invalid council address";
        if (errorCode == ERR_SYSSTATE_FUNCTION_PAUSED)
            return "SystemState: Function is paused";

        // QC Manager errors
        if (errorCode == ERR_QCMGR_INVALID_QC_ADDR)
            return "QCManager: Invalid QC address";
        if (errorCode == ERR_QCMGR_INVALID_CAPACITY)
            return "QCManager: Invalid minting capacity";
        if (errorCode == ERR_QCMGR_QC_ALREADY_REGISTERED)
            return "QCManager: QC already registered";
        if (errorCode == ERR_QCMGR_ACTION_ALREADY_QUEUED)
            return "QCManager: Action already queued";
        if (errorCode == ERR_QCMGR_ACTION_NOT_QUEUED)
            return "QCManager: Action not queued";
        if (errorCode == ERR_QCMGR_DELAY_NOT_ELAPSED)
            return "QCManager: Delay period not elapsed";
        if (errorCode == ERR_QCMGR_ACTION_ALREADY_EXECUTED)
            return "QCManager: Action already executed";
        if (errorCode == ERR_QCMGR_QC_NOT_REGISTERED)
            return "QCManager: QC not registered";
        if (errorCode == ERR_QCMGR_INVALID_WALLET_ADDR)
            return "QCManager: Invalid wallet address";
        if (errorCode == ERR_QCMGR_QC_NOT_ACTIVE)
            return "QCManager: QC not active";
        if (errorCode == ERR_QCMGR_SPV_VERIFICATION_FAILED)
            return "QCManager: SPV verification failed";
        if (errorCode == ERR_QCMGR_WALLET_NOT_REGISTERED)
            return "QCManager: Wallet not registered";
        if (errorCode == ERR_QCMGR_NOT_AUTHORIZED)
            return "QCManager: Not authorized";
        if (errorCode == ERR_QCMGR_WALLET_NOT_ACTIVE)
            return "QCManager: Wallet not active";
        if (errorCode == ERR_QCMGR_SOLVENCY_NOT_AUTHORIZED)
            return "QCManager: Not authorized for solvency check";
        if (errorCode == ERR_QCMGR_QC_WOULD_BE_INSOLVENT)
            return "QCManager: QC would become insolvent";

        // QC Minter errors
        if (errorCode == ERR_MINTER_INVALID_QC_ADDR)
            return "QCMinter: Invalid QC address";
        if (errorCode == ERR_MINTER_INVALID_AMOUNT)
            return "QCMinter: Invalid amount";
        if (errorCode == ERR_MINTER_MINTING_PAUSED)
            return "QCMinter: Minting is paused";
        if (errorCode == ERR_MINTER_POLICY_NOT_AVAILABLE)
            return "QCMinter: Minting policy not available";
        if (errorCode == ERR_MINTER_POLICY_VALIDATION_FAILED)
            return "QCMinter: Policy validation failed";
        if (errorCode == ERR_MINTER_TOKEN_MINT_FAILED)
            return "QCMinter: Token mint failed";

        // QC Redeemer errors
        if (errorCode == ERR_REDEEMER_INVALID_QC_ADDR)
            return "QCRedeemer: Invalid QC address";
        if (errorCode == ERR_REDEEMER_INVALID_AMOUNT)
            return "QCRedeemer: Invalid amount";
        if (errorCode == ERR_REDEEMER_REDEMPTION_FAILED)
            return "QCRedeemer: Redemption request failed";
        if (errorCode == ERR_REDEEMER_NOT_PENDING)
            return "QCRedeemer: Redemption not pending";
        if (errorCode == ERR_REDEEMER_FULFILLMENT_FAILED)
            return "QCRedeemer: Fulfillment verification failed";
        if (errorCode == ERR_REDEEMER_DEFAULT_FAILED)
            return "QCRedeemer: Default flagging failed";
    ...
    function getContractFromErrorCode(uint16 errorCode)
        external
        pure
    ...
    function isSecurityCritical(uint16 errorCode)
        external
        pure
