...
contract QCReserveLedger is AccessControl {
    ...
    function submitReserveAttestation(
        ...
        uint256 oldBalance = hasAttestation[qc] ? reserveAttestations[qc].balance : 0;
    ...
    function getTimeUntilStale(address qc) external view returns (uint256 timeUntilStale) {
        ReserveAttestation memory attestation = reserveAttestations[qc];
    ...
    function invalidateAttestation(
        ...
        require(reserveAttestations[qc].timestamp != 0, "No attestation exists");
        ...
        returns (bool[] memory staleFlags) 
    {
        ...
        for (uint256 i = 0; i < qcs.length; i++) {
            ...
            if (!attestation.isValid || attestation.timestamp == 0) {
                ...
                staleFlags[i] = block.timestamp > attestation.timestamp + staleThreshold;
            }
        }
    ...
    function getAttestationSummary() 
        external 
        view 
        returns (uint256 totalQCs, uint256 totalBalance, uint256 staleCount) 
    {
