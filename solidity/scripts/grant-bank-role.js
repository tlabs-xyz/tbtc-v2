...
async function main() {
  ...
  try {
    ...
    const hasRole = await bank.hasRole(BALANCE_INCREASER_ROLE, QC_MINTER_ADDRESS);
    ...
    const hasRoleAfter = await bank.hasRole(BALANCE_INCREASER_ROLE, QC_MINTER_ADDRESS);
    if (hasRoleAfter) {
