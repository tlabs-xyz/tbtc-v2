...
describe("Cross-Contract Edge Cases", () => {
  ...
  describe("Inter-Contract State Synchronization", () => {
    it("should handle Bank balance updates during concurrent operations", async () => {
      ...
      await mintingPolicy
        .connect(governance)
        .setQCCapacity(qcAddress.address, ethers.utils.parseUnits("200", 18))
    ...
    it("should maintain consistency when Bridge updates wallet states", async () => {
      ...
      const walletPubKeyHash = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes("wallet1")
      )
      await qcProxy
        .connect(qcAddress)
        .registerBitcoinWallet(
          walletPubKeyHash,
      ...
      await mintingPolicy
        .connect(governance)
        .setQCCapacity(qcAddress.address, ethers.utils.parseUnits("50", 18))
  ...
  describe("Multi-Contract Transaction Atomicity", () => {
    ...
    it("should handle gas limit edge cases across contracts", async () => {
      ...
      await mintingPolicy
        .connect(governance)
        .setQCCapacity(qcAddress.address, largeAmount)
