...
describe("Cross-Contract Real Integration - Advanced Scenarios", () => {
  ...
  async function deployAndConfigureRealContracts() {
    ...
    protocolRegistry = await (
      await ethers.getContractFactory("ProtocolRegistry")
    ).deploy()
    systemState = await (
      await ethers.getContractFactory("SystemState")
    ).deploy()
    qcData = await (await ethers.getContractFactory("QCData")).deploy()
    qcReserveLedger = await (
      await ethers.getContractFactory("QCReserveLedger")
    ).deploy()
    tbtc = await (await ethers.getContractFactory("TBTC")).deploy()
    spvValidator = await (
      await ethers.getContractFactory("SPVValidator")
    ).deploy()
    ...
    basicMintingPolicy = await (
      await ethers.getContractFactory("BasicMintingPolicy")
    ).deploy(protocolRegistry.address)
    basicRedemptionPolicy = await (
      await ethers.getContractFactory("BasicRedemptionPolicy")
    ).deploy(protocolRegistry.address)
    qcMinter = await (
      await ethers.getContractFactory("QCMinter")
    ).deploy(protocolRegistry.address)
    qcRedeemer = await (
      await ethers.getContractFactory("QCRedeemer")
    ).deploy(protocolRegistry.address)
  ...
  async function setupQCWithReserves(
    qcAddress: string,
    btcAddress: string,
    reserveAmount = TEST_AMOUNTS.RESERVE
  ) {
    await qcData.registerQC(qcAddress)
    await qcManager.registerWallet(qcAddress, btcAddress)
    await qcReserveLedger
      .connect(watchdog)
      .submitAttestation(qcAddress, reserveAmount)
  ...
  describe("State Synchronization Stress Tests", () => {
    ...
    it("should maintain state consistency during rapid status changes", async () => {
      ...
      expect(
        await basicMintingPolicy.getAvailableMintingCapacity(qc1.address)
      ).to.be.gt(0)
      ...
      const statusChanges = [
        () =>
          qcManager
            .connect(watchdog)
            .setQCStatus(qc1.address, 1, ethers.utils.id("REVIEW_1")), // UnderReview
        () =>
          qcManager
            .connect(watchdog)
            .setQCStatus(qc1.address, 0, ethers.utils.id("ACTIVE_1")), // Back to Active
        () =>
          qcManager
            .connect(watchdog)
            .setQCStatus(qc1.address, 1, ethers.utils.id("REVIEW_2")), // UnderReview again
        () =>
          qcManager
            .connect(watchdog)
            .setQCStatus(qc1.address, 2, ethers.utils.id("REVOKE_1")), // Revoked
      ]
    ...
    it("should handle reserve updates affecting multiple contract states", async () => {
      ...
      await qcReserveLedger
        .connect(watchdog)
        .submitAttestation(qc1.address, lowReserves)

      // Verify capacity updated across system
      const newCapacity = await basicMintingPolicy.getAvailableMintingCapacity(
        qc1.address
      )
  ...
  describe("Complex Transaction Ordering", () => {
    beforeEach(async () => {
      await setupQCWithReserves(
        qc1.address,
        TEST_BTC_ADDRESSES.QC1,
        TEST_AMOUNTS.MEDIUM.mul(3)
      )
    })
    ...
    it("should handle interleaved mint and emergency pause operations", async () => {
      ...
      const mintPromise = qcMinter
        .connect(user1)
        .requestQCMint(qc1.address, TEST_AMOUNTS.MEDIUM)

      // Immediately pause the QC (this should not affect the ongoing mint if it's already committed)
      const pausePromise = systemState
        .connect(watchdog)
        .emergencyPauseQC(qc1.address, ethers.utils.id("EMERGENCY"))
    ...
    it("should handle concurrent capacity exhaustion scenarios", async () => {
      ...
      await qcReserveLedger
        .connect(watchdog)
        .submitAttestation(qc1.address, exactCapacity)
  ...
  describe("Gas Cost Analysis for Complex Operations", () => {
    ...
    it("should measure gas costs for complete mint-to-redemption cycle", async () => {
      ...
      gasResults.redemptionFulfillment = (
        await fulfillmentTx.wait()
      ).gasUsed.toNumber()
      ...
      console.log(
        `Redemption Initiation: ${gasResults.redemptionInitiation.toLocaleString()} gas`
      )
      console.log(
        `Redemption Fulfillment: ${gasResults.redemptionFulfillment.toLocaleString()} gas`
      )
      console.log(
        `Total Cycle: ${Object.values(gasResults)
          .reduce((a, b) => a + b, 0)
          .toLocaleString()} gas`
      )
    ...
    it("should verify gas costs don't increase with system complexity", async () => {
      ...
      console.log(
        `Increase: ${(
          (gasComplexState.toNumber() / gas1QC.toNumber() - 1) *
          100
        ).toFixed(1)}%`
      )
    })
  ...
  describe("System Recovery and Resilience", () => {
    ...
    it("should recover gracefully from multiple simultaneous emergencies", async () => {
      ...
      await qcMinter
        .connect(user2)
        .requestQCMint(qc2.address, TEST_AMOUNTS.MEDIUM)

      // Simultaneous emergencies
      const emergencyPromises = [
        systemState
          .connect(watchdog)
          .emergencyPauseQC(qc1.address, ethers.utils.id("EMERGENCY_1")),
        systemState
          .connect(watchdog)
          .emergencyPauseQC(qc2.address, ethers.utils.id("EMERGENCY_2")),
      ]
    ...
    it("should maintain data integrity during system stress", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        stressOperations.push(
          ...
          qcReserveLedger
            .connect(watchdog)
            .submitAttestation(qc1.address, TEST_AMOUNTS.RESERVE.add(i)),
          qcReserveLedger
            .connect(watchdog)
            .submitAttestation(qc2.address, TEST_AMOUNTS.RESERVE.add(i * 2))
        )
      }
      ...
      expect(
        await basicMintingPolicy.getAvailableMintingCapacity(qc2.address)
      ).to.be.gt(0)

      console.log(
        `Final user1 balance: ${ethers.utils.formatEther(qc1Balance)} tBTC`
      )
      console.log(
        `Final user2 balance: ${ethers.utils.formatEther(qc2Balance)} tBTC`
      )
