...
describe("Real Contract Integration - Phase 1 & 2", () => {
  ...
  async function setupQC(
    qcAddress: string,
    btcAddress: string,
    reserveBalance = TEST_RESERVE_BALANCE
  ) {
    // Register QC
  ...
  describe("Phase 1: End-to-End User Journeys", () => {
    ...
    it("should complete full mint-to-redemption lifecycle with real contracts", async () => {
      ...
      const mintTx = await qcMinter
        .connect(user1)
        .requestQCMint(qc1.address, TEST_MINT_AMOUNT)
      ...
      const redemptionTx = await qcRedeemer
        .connect(user1)
        .initiateRedemption(qc1.address, TEST_MINT_AMOUNT, TEST_BTC_ADDRESS)
      ...
      expect(await basicRedemptionPolicy.isRedemptionFulfilled(redemptionId)).to
        .be.true
  ...
  describe("Phase 1: Emergency Response Flows", () => {
    ...
    it("should handle QC emergency pause with real cross-contract effects", async () => {
      ...
      await systemState
        .connect(watchdog)
        .emergencyPauseQC(qc1.address, pauseReason)
  ...
  describe("Phase 2: Multi-QC Scenarios", () => {
    ...
    it("should handle capacity exhaustion across multiple QCs", async () => {
      ...
      const qc3Capacity = await basicMintingPolicy.getAvailableMintingCapacity(
        qc3.address
      )
      ...
      expect(
        await basicMintingPolicy.getAvailableMintingCapacity(qc3.address)
      ).to.equal(0)
      ...
      await expect(
        qcMinter
          .connect(user1)
          .requestQCMint(qc3.address, ethers.utils.parseEther("1"))
      ).to.be.reverted
    ...
    it("should handle QC status changes affecting system-wide behavior", async () => {
      ...
      await qcManager
        .connect(watchdog)
        .setQCStatus(qc2.address, 1, reviewReason) // UnderReview
      ...
      await qcManager
        .connect(watchdog)
        .setQCStatus(qc2.address, 2, revokeReason) // Revoked
