 ...
 * Tests both QCManagerSPV and QCRedeemerSPV with real Bitcoin transaction data
...
describe("SPV Libraries Integration", () => {
  ...
  // Real Bitcoin mainnet transaction data from Bridge tests
  ...
  const validBitcoinTx = {
    name: "ValidMainnetTransaction",
    description: "Real Bitcoin mainnet transaction with complete SPV proof",
    txInfo: {
      version: "0x01000000",
      inputVector:
  ...
  before(async () => {
    ...
    // Set up relay with appropriate difficulty from Bitcoin headers
    await testRelay.setCurrentEpochDifficultyFromHeaders(
      validBitcoinTx.proof.bitcoinHeaders
    )
    await testRelay.setPrevEpochDifficultyFromHeaders(
      validBitcoinTx.proof.bitcoinHeaders
    )

    // Configure TestRelay to return successful validation
    await testRelay.setValidateHeaderChainResult(1000000) // High difficulty value indicating success
  })
  ...
  describe("SharedSPVCore Integration", () => {
    it("should validate real Bitcoin transaction using SharedSPVCore", async () => {
      ...
      expect(result).to.equal(validBitcoinTx.expectedTxHash)
    })
    ...
    it("should decode Bitcoin addresses correctly", async () => {
      ...
      expect(scriptHash.length).to.be.greaterThan(2) // Should have decoded data
    })
  })
  ...
  describe("QCRedeemerSPV Integration", () => {
    ...
    it("should validate SPV proof using SharedSPVCore", async () => {
      const txHash = await qcRedeemerSPVTest.validateSPVProof(
        validBitcoinTx.txInfo,
        validBitcoinTx.proof
      )

      expect(txHash).to.equal(validBitcoinTx.expectedTxHash)
    })
  ...
  describe("Library Efficiency Comparison", () => {
    ...
    it("should verify deployment gas costs", async () => {
      // These values are captured from actual deployment
