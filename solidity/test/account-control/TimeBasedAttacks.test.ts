...
describe("Time-Based Attack Scenarios", () => {
  ...
  describe("Front-Running and MEV Attacks", () => {
    ...
    it("should handle sandwich attacks on redemption operations", async () => {
      ...
      await expect(
        qcRedeemer
          .connect(attacker)
          .initiateRedemption(ethers.utils.parseUnits("10", 18), validLegacyBtc)
  ...
  describe("Attestation Timing Attacks", () => {
    it("should prevent stale attestation exploitation", async () => {
      ...
      await qcData
        .connect(governance)
        .updateAttestation(qcAddress.address, reserves, attestationTime)
  ...
  describe("Time Window Exploitation", () => {
    ...
    it("should handle emergency pause timing windows", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .submitCriticalReport(qcAddress.address, "Issue 1")
      ...
      await watchdogMonitor
        .connect(watchdog2)
        .submitCriticalReport(qcAddress.address, "Issue 2")

      // Third report triggers pause
      await watchdogMonitor
        .connect(watchdog3)
        .submitCriticalReport(qcAddress.address, "Issue 3")
      ...
      const pauseDetails = await watchdogMonitor.emergencyPauses(
        qcAddress.address
      )
