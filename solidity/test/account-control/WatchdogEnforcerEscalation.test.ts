...
describe("WatchdogEnforcer Escalation", () => {
  ...
  beforeEach(async () => {
    ...
    mockReserveLedger.getLastAttestationTime.returns(Math.floor(Date.now() / 1000))
  })
  ...
  describe("Escalation Timer Mechanism", () => {
    it("should start escalation timer for INSUFFICIENT_RESERVES violations", async () => {
      ...
      const tx = await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, INSUFFICIENT_RESERVES)
      const receipt = await tx.wait()
      const timestamp = (await ethers.provider.getBlock(receipt.blockNumber)).timestamp

      // Should start escalation timer
      expect(await watchdogEnforcer.criticalViolationTimestamps(qcAddress.address)).to.equal(timestamp)

      // Should emit CriticalViolationDetected event
      await expect(tx)
        .to.emit(watchdogEnforcer, "CriticalViolationDetected")
        .withArgs(qcAddress.address, INSUFFICIENT_RESERVES, deployer.address, timestamp, timestamp + ESCALATION_DELAY)
    })

    it("should NOT start escalation timer for STALE_ATTESTATIONS violations", async () => {
      // Set up stale attestations
      mockReserveLedger.getLastAttestationTime.returns(Math.floor(Date.now() / 1000) - (8 * 24 * 60 * 60)) // 8 days old

      const tx = await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, STALE_ATTESTATIONS)

      // Should NOT start escalation timer
      expect(await watchdogEnforcer.criticalViolationTimestamps(qcAddress.address)).to.equal(0)

      // Should NOT emit CriticalViolationDetected event
      await expect(tx).to.not.emit(watchdogEnforcer, "CriticalViolationDetected")
    })

    it("should set QC to UnderReview for both violation types", async () => {
      // Test INSUFFICIENT_RESERVES
      await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, INSUFFICIENT_RESERVES)
      ...
      mockReserveLedger.getLastAttestationTime.returns(Math.floor(Date.now() / 1000) - (8 * 24 * 60 * 60))

      await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, STALE_ATTESTATIONS)
      expect(mockQcManager.requestStatusChange).to.have.been.calledWith(
        qcAddress.address,
        1, // UnderReview  
        STALE_ATTESTATIONS
  ...
  describe("checkEscalation Function", () => {
    ...
    it("should escalate to emergency pause after delay", async () => {
      ...
      const tx = await watchdogEnforcer.connect(watchdog).checkEscalation(qcAddress.address)
      ...
      await expect(tx)
        ...
        .withArgs(qcAddress.address, SUSTAINED_RESERVE_VIOLATION, watchdog.address, await helpers.time.latest())
    ...
    it("should clear timer if QC status changed to Active", async () => {
      ...
      await expect(tx)
        ...
        .withArgs(qcAddress.address, deployer.address, await helpers.time.latest())
  ...
  describe("clearEscalationTimer Function", () => {
    ...
    it("should clear timer when QC returns to Active status", async () => {
      ...
      const tx = await watchdogEnforcer.connect(watchdog).clearEscalationTimer(qcAddress.address)
      ...
      await expect(tx)
        .to.emit(watchdogEnforcer, "EscalationTimerCleared")
        .withArgs(qcAddress.address, watchdog.address, await helpers.time.latest())
    })
    ...
    it("should NOT clear timer if QC is still UnderReview", async () => {
      ...
      expect(await watchdogEnforcer.criticalViolationTimestamps(qcAddress.address)).to.not.equal(0)
    })
    ...
    it("should NOT clear timer if no timer exists", async () => {
      ...
      expect(await watchdogEnforcer.criticalViolationTimestamps(thirdParty.address)).to.equal(0)
    })
    ...
    it("should allow anyone to call clearEscalationTimer", async () => {
      ...
      await expect(
        watchdogEnforcer.connect(thirdParty).clearEscalationTimer(qcAddress.address)
      ).to.not.be.reverted
    })
  })
  ...
  describe("Integration with Emergency Pause System", () => {
    it("should successfully emergency pause QC through escalation", async () => {
      ...
      expect(await systemState.isQCEmergencyPaused(qcAddress.address)).to.be.true
      expect(await systemState.getQCPauseTimestamp(qcAddress.address)).to.be.gt(0)
    })
    ...
    it("should use SUSTAINED_RESERVE_VIOLATION reason code", async () => {
      ...
      await expect(tx)
        .to.emit(watchdogEnforcer, "ViolationEscalated")
        .withArgs(qcAddress.address, SUSTAINED_RESERVE_VIOLATION, deployer.address, await helpers.time.latest())
    })
    ...
    it("should require PAUSER_ROLE for escalation to work", async () => {
      ...
      await expect(
        watchdogEnforcer.checkEscalation(qcAddress.address)
      ).to.be.revertedWith(`AccessControl: account ${watchdogEnforcer.address.toLowerCase()} is missing role ${PAUSER_ROLE}`)
    })
  })
  ...
  describe("Edge Cases and Error Conditions", () => {
    it("should handle multiple QCs with different escalation timers", async () => {
      ...
      await watchdogEnforcer.enforceObjectiveViolation(qc2.address, INSUFFICIENT_RESERVES)
      
      await helpers.time.increase(10 * 60) // 10 minutes later  
      await watchdogEnforcer.enforceObjectiveViolation(qc3.address, INSUFFICIENT_RESERVES)

      // Verify different timestamps
      const timestamp1 = await watchdogEnforcer.criticalViolationTimestamps(qcAddress.address)
      const timestamp2 = await watchdogEnforcer.criticalViolationTimestamps(qc2.address)
      const timestamp3 = await watchdogEnforcer.criticalViolationTimestamps(qc3.address)
    ...
    it("should handle re-triggering violation for same QC", async () => {
      ...
      const tx1 = await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, INSUFFICIENT_RESERVES)
      const receipt1 = await tx1.wait()
      const initialTimestamp = (await ethers.provider.getBlock(receipt1.blockNumber)).timestamp

      // Wait some time and trigger again
      await helpers.time.increase(10 * 60) // 10 minutes
      const tx2 = await watchdogEnforcer.enforceObjectiveViolation(qcAddress.address, INSUFFICIENT_RESERVES)
      const receipt2 = await tx2.wait()
      const newTimestamp = (await ethers.provider.getBlock(receipt2.blockNumber)).timestamp

      // Should update the timestamp
      expect(await watchdogEnforcer.criticalViolationTimestamps(qcAddress.address)).to.equal(newTimestamp)
  ...
  describe("Constants and Configuration", () => {
    ...
    it("should have correct reason codes", async () => {
      expect(await watchdogEnforcer.INSUFFICIENT_RESERVES()).to.equal(INSUFFICIENT_RESERVES)
      expect(await watchdogEnforcer.STALE_ATTESTATIONS()).to.equal(STALE_ATTESTATIONS) 
      expect(await watchdogEnforcer.SUSTAINED_RESERVE_VIOLATION()).to.equal(SUSTAINED_RESERVE_VIOLATION)
    })
  })
})