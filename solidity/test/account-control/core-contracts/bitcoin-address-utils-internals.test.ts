  ...
  describe("Base58 Decoding", () => {
    ...
    describe("Valid Base58 Decoding", () => {
      it("should decode valid base58 address correctly", async () => {
        const inputBytes = ethers.utils.toUtf8Bytes(base58TestVectors.valid.simple.input)
        const decoded = await testInternals.testBase58Decode(inputBytes)
        
        expect(ethers.utils.arrayify(decoded).length).to.equal(base58TestVectors.valid.simple.expectedLength)
        expect(decoded).to.not.equal("0x")
      })

      it("should handle leading zeros correctly", async () => {
        const inputBytes = ethers.utils.toUtf8Bytes(base58TestVectors.valid.leadingZeros.input)
        const decoded = await testInternals.testBase58Decode(inputBytes)
      ...
      it("should decode P2SH addresses correctly", async () => {
        const inputBytes = ethers.utils.toUtf8Bytes(base58TestVectors.valid.multiByte.input)
        const decoded = await testInternals.testBase58Decode(inputBytes)
        
        expect(ethers.utils.arrayify(decoded).length).to.equal(base58TestVectors.valid.multiByte.expectedLength)
      })
    })

    describe("Invalid Base58 Handling", () => {
      it("should reject base58 with invalid characters", async () => {
        const inputBytes = ethers.utils.toUtf8Bytes(base58TestVectors.invalid.invalidCharacter)
        await expect(
      ...
      it("should handle empty input", async () => {
        const inputBytes = ethers.utils.toUtf8Bytes(base58TestVectors.invalid.empty)
        const decoded = await testInternals.testBase58Decode(inputBytes)
    ...
    describe("Base58 Character Conversion", () => {
      ...
      it("should convert valid base58 characters correctly", async () => {
        for (let i = 0; i < base58Alphabet.length; i++) {
          ...
          const result = await testInternals.testBase58CharToValue(ethers.utils.hexlify([charByte]))
      ...
      it("should reject invalid base58 characters", async () => {
        ...
        for (const char of invalidChars) {
          const charByte = ethers.utils.toUtf8Bytes(char)[0]
          await expect(
            testInternals.testBase58CharToValue(ethers.utils.hexlify([charByte]))
          ).to.be.revertedWithCustomError(testInternals, "InvalidAddressPrefix")
        }
      })
    })
  ...
  describe("Bech32 Operations", () => {
    ...
    describe("Bech32 Address Detection", () => {
      it("should detect valid bech32 addresses", async () => {
        for (const [type, address] of Object.entries(bech32TestVectors.validAddresses)) {
      ...
      it("should reject mixed case bech32 addresses", async () => {
        const addressBytes = ethers.utils.toUtf8Bytes(bech32TestVectors.invalidAddresses.mixedCase)
        const result = await testInternals.testIsBech32Address(addressBytes)
        expect(result).to.be.false
      })

      it("should reject addresses with wrong prefix", async () => {
        const addressBytes = ethers.utils.toUtf8Bytes(bech32TestVectors.invalidAddresses.wrongPrefix)
        const result = await testInternals.testIsBech32Address(addressBytes)
        expect(result).to.be.false
      })

      it("should reject addresses without proper separator", async () => {
        const addressBytes = ethers.utils.toUtf8Bytes(bech32TestVectors.invalidAddresses.noSeparator)
        const result = await testInternals.testIsBech32Address(addressBytes)
        expect(result).to.be.false
      })
    })
    ...
    describe("Bech32 Character Conversion", () => {
      ...
      it("should convert valid bech32 characters correctly", async () => {
        for (let i = 0; i < bech32Charset.length; i++) {
          ...
          const charByte = ethers.utils.toUtf8Bytes(char)[0]
          const result = await testInternals.testBech32CharToValue(ethers.utils.hexlify([charByte]))
          expect(result).to.equal(i)
        }
      })
      ...
      it("should reject invalid bech32 characters", async () => {
        ...
        for (const char of invalidChars) {
          const charByte = ethers.utils.toUtf8Bytes(char)[0]
          await expect(
            testInternals.testBech32CharToValue(ethers.utils.hexlify([charByte]))
          ).to.be.revertedWithCustomError(testInternals, "InvalidAddressPrefix")
        }
      })
    })
  ...
  describe("Bit Conversion", () => {
    describe("5-bit to 8-bit Conversion", () => {
      ...
      it("should handle padding correctly", async () => {
        ...
        expect(ethers.utils.arrayify(resultWithPad).length).to.be.greaterThanOrEqual(ethers.utils.arrayify(resultWithoutPad).length)
      })
    ...
    describe("8-bit to 5-bit Conversion", () => {
      it("should convert 8-bit data to 5-bit groups", async () => {
        ...
        expect(ethers.utils.arrayify(result).length).to.be.greaterThan(input8bit.length) // More 5-bit groups than 8-bit bytes
      })
    })
  ...
  describe("Address Decoding Components", () => {
    describe("Base58 Address Decoding", () => {
      ...
      it("should reject invalid base58 address", async () => {
        ...
        await expect(
          testInternals.testDecodeBase58Address(addressBytes)
        ).to.be.reverted
      })
    })
    ...
    describe("Bech32 Address Decoding", () => {
      ...
      it("should reject invalid bech32 address", async () => {
        ...
        await expect(
          testInternals.testDecodeBech32Address(addressBytes)
        ).to.be.reverted
      })
    })
  })
  ...
  describe("Checksum Functions", () => {
    describe("Bech32 Checksum Calculation", () => {
      it("should calculate checksum for derivation correctly", async () => {
        const hrp = "bc"
        const testData = [0, 14, 20, 15, 7, 13, 26, 0, 25, 18, 6, 11, 13, 7, 21, 16, 10, 29, 3, 6, 27, 15, 14, 26, 12, 10, 11, 15, 7]
      ...
      it("should produce different checksums for different data", async () => {
        ...
        const checksum1 = await testInternals.testBech32ChecksumForDerivation(hrp, data1, data1.length)
        const checksum2 = await testInternals.testBech32ChecksumForDerivation(hrp, data2, data2.length)
      ...
      it("should produce different checksums for different HRP", async () => {
        ...
        const checksumBC = await testInternals.testBech32ChecksumForDerivation("bc", data, data.length)
        const checksumTB = await testInternals.testBech32ChecksumForDerivation("tb", data, data.length)
