...
describe("QCRedeemer - Wallet Obligations", () => {
  ...
  beforeEach(async () => {
    ...
    // Deploy mock AccountControl
    const MockAccountControlFactory = await ethers.getContractFactory(
      "MockAccountControl"
    )

    const mockBank = await smock.fake("MockBank")
    ...
    const deployedMockBank = await MockBankFactory.deploy()

    // Deploy MockAccountControl and configure QCRedeemer with comprehensive setup
    const NewMockAccountControlFactory = await ethers.getContractFactory(
      "MockAccountControl"
    )

    mockAccountControl = await NewMockAccountControlFactory.deploy(
      deployedMockBank.address
    )
