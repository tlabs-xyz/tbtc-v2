...
export async function resetAccountControlState(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(signers.length, 10); i++) {
      try {
        ...
        if (isAuthorized) {
          await accountControl
            .connect(owner)
            .deauthorizeReserve(signers[i].address)
    ...
    try {
      ...
      if (
        currentEmergencyCouncil.toLowerCase() !==
        emergencyCouncil.address.toLowerCase()
      ) {
        await accountControl
          .connect(owner)
...
export async function resetOracleState(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(signers.length, 10); i++) {
      try {
        const attestation = await reserveOracle.getCurrentAttestation(
          signers[i].address
        )
  ...
  try {
    ...
    try {
      ...
      if (currentAccountControl !== ethers.constants.AddressZero) {
        await qcManager
          .connect(owner)
          .setAccountControl(ethers.constants.AddressZero)
...
export async function createEnhancedSnapshot(
  ...
  const { createSnapshot } = await import("hardhat").then(
    (hh) => hh.helpers.snapshot
  )
...
export async function restoreEnhancedSnapshot() {
  const { restoreSnapshot } = await import("hardhat").then(
    (hh) => hh.helpers.snapshot
  )
  ...
  try {
    ...
    if (contracts.accountControl) {
      ...
      if (owner.toLowerCase() !== signers[0].address.toLowerCase()) {
        issues.push(
          `AccountControl owner is ${owner}, expected ${signers[0].address}`
        )
      }
    }
    ...
    if (contracts.qcManager) {
      ...
      if (
        accountControl !== ethers.constants.AddressZero &&
        !contracts.accountControl
      ) {
        issues.push(
          "QCManager has AccountControl reference but test doesn't expect it"
        )
      }
    }
