...
describe("End-to-End User Journeys", () => {
  ...
  describe("Happy Path: Institutional User Journey", () => {
    it("should handle complete institutional minting and redemption flow", async () => {
      ...
      const updatedReserves = QC_RESERVE_BALANCE.add(
      ...
      await qcRedeemer.connect(governance).recordRedemptionFulfillment(
        redemptionId,
        userBtcAddress,
      ...
      const finalBalance =
        LARGE_MINT_AMOUNT.sub(redemptionAmount).add(MEDIUM_MINT_AMOUNT)
      expect(await tbtc.balanceOf(institutionalUser.address)).to.equal(
        finalBalance
      )
    })
  ...
  describe("Retail User Journey: Small Transactions", () => {
    it("should handle retail user minting and redemption with small amounts", async () => {
      ...
      const expectedTotalSupply =
        LARGE_MINT_AMOUNT.add(totalRetailMinted).sub(redemptionAmount)
    ...
    it("should handle QC investigation and recovery process", async () => {
      ...
      const recoveryProposalData = ethers.utils.defaultAbiCoder.encode(
        ...
        [
          qcAddress.address,
          QCStatus.Active,
          ethers.utils.id("INVESTIGATION_CLEARED"),
        ]
      )
      ...
      await watchdogConsensusManager
        .connect(watchdog1)
        .executeProposal(recoveryProposalId)
  ...
  describe("Worst Case Scenario: QC Insolvency and Revocation", () => {
    ...
    it("should handle QC insolvency detection and revocation process", async () => {
      const totalMinted =
        LARGE_MINT_AMOUNT.add(MEDIUM_MINT_AMOUNT).add(MEDIUM_MINT_AMOUNT)
      ...
      for (let i = 0; i < 3; i++) {
        await watchdogMonitor
          ...
          .submitCriticalReport(
            qcAddress.address,
            `CRITICAL INSOLVENCY: Reserves ${criticalReserves.div(
              ethers.utils.parseEther("1")
            )} BTC vs Minted ${totalMinted.div(
              ethers.utils.parseEther("1")
            )} tBTC`
          )
      }
      ...
      const revocationProposalData = ethers.utils.defaultAbiCoder.encode(
        ...
        [
          qcAddress.address,
          QCStatus.Revoked,
          ethers.utils.id("INSOLVENCY_REVOCATION"),
        ]
      )
      ...
      const revocationEvent = revocationReceipt.events?.find(
        (e) => e.event === "ProposalCreated"
      )
      ...
      await watchdogConsensusManager
        .connect(watchdog1)
        .executeProposal(revocationProposalId)
  ...
  describe("Attack Scenario: Malicious Actor Attempts", () => {
    ...
    it("should prevent malicious actor from exploiting the system", async () => {
      ...
      await expect(
        qcQCReserveLedger.connect(maliciousActor).submitReserveAttestation(
          qcAddress.address,
  ...
  describe("System Upgrade and Migration Journey", () => {
    it("should handle system upgrades while maintaining user operations", async () => {
      ...
      expect(totalSupplyBefore).to.equal(
        LARGE_MINT_AMOUNT.add(MEDIUM_MINT_AMOUNT)
      )
      ...
      expect(await tbtc.balanceOf(retailUser.address)).to.be.gt(
        MEDIUM_MINT_AMOUNT
      ) // Includes new mint
