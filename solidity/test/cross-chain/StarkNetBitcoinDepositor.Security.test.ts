...
describe("StarkNetBitcoinDepositor - Security & Edge Cases", () => {
  ...
  // let snapshotId: string // Store snapshot ID
  ...
  describe("Reentrancy Protection", () => {
    it("should prevent reentrancy attacks during finalizeDeposit", async () => {
      ...
      const reentrancyAttackerContract = await ReentrancyAttacker.deploy(
        depositor.address
      )

      // Attempt reentrancy attack
      await expect(
        reentrancyAttackerContract.attack({ value: INITIAL_MESSAGE_FEE })
  ...
  describe("Access Control", () => {
    ...
    it("should only allow owner to update fee buffer", async () => {
      ...
      // Owner should succeed
      await expect(depositor.connect(owner).updateFeeBuffer(newBuffer))
  ...
  describe("Integer Overflow/Underflow Protection", () => {
    it("should handle maximum uint256 amounts safely", async () => {
      ...
      await expect(depositor.connect(owner).updateL1ToL2MessageFee(maxUint256))
        .to.emit(depositor, "L1ToL2MessageFeeUpdated")
        .withArgs(maxUint256)
    ...
    it("should validate message fee payment", async () => {
      ...
      await expect(
        depositor.finalizeDeposit(depositKey, {
          value: INITIAL_MESSAGE_FEE.sub(1),
        })
      ).to.be.revertedWith("Insufficient L1->L2 message fee")
    })
  })
  ...
  describe("Front-running Protection", () => {
    ...
    it("should handle deposit finalization atomically", async () => {
      ...
      expect(await tbtcToken.balanceOf(depositor.address)).to.equal(
        initialBalance
      )
