...
contract TestSetup is Test {
    ...
    bytes32 public constant TEST_L2_RECIPIENT = bytes32(uint256(0x0123456789abcdef));
    ...
    function initializeTestDeposit() public returns (bytes32 depositKey) {
        ...
        depositor.initializeDeposit(defaultFundingTx, defaultReveal, TEST_L2_RECIPIENT);
        return depositKey;
    }
    ...
    function calculateDepositKey(
        ...
        bytes32 fundingTxHash = keccak256(abi.encodePacked(
            fundingTx.version,
            fundingTx.inputVector,
            fundingTx.outputVector,
            fundingTx.locktime
        ));
        return bytes32(uint256(keccak256(abi.encodePacked(fundingTxHash, reveal.fundingOutputIndex))));
    }
    ...
    function generateTestFundingTx(uint256 seed) public pure returns (IBridgeTypes.BitcoinTxInfo memory) {
    ...
    function generateTestReveal(address vault, uint256 seed) public pure returns (IBridgeTypes.DepositRevealInfo memory) {
        return IBridgeTypes.DepositRevealInfo({
            fundingOutputIndex: uint32(seed),
            blindingFactor: bytes8(uint64(seed)),
            walletPubKeyHash: bytes20(keccak256(abi.encode("wallet", seed))),
            refundPubKeyHash: bytes20(keccak256(abi.encode("refund", seed))),
    ...
    function startGasMeasurement(string memory operation) public returns (GasMeasurement memory) {
    ...
    function endGasMeasurement(GasMeasurement memory measurement) public view returns (uint256 gasUsed) {
        gasUsed = measurement.gasStart - gasleft();
        return gasUsed;
    }
    
    // Event testing helpers
    function expectDepositInitializedEvent(bytes32 depositKey, bytes32 l2Recipient) public {
    ...
    function expectTBTCBridgedEvent(
        ...
        vm.expectEmit(true, true, true, true);
        emit TBTCBridgedToStarkNet(depositKey, uint256(l2Recipient), amount, messageNonce);
    }
    ...
    // Events
    event DepositInitializedForStarkNet(bytes32 indexed depositKey, uint256 indexed starkNetRecipient);
    event TBTCBridgedToStarkNet(bytes32 indexed depositKey, uint256 indexed starkNetRecipient, uint256 amount, uint256 messageNonce);
    event L1ToL2MessageFeeUpdated(uint256 newFee);
    event StarkNetBitcoinDepositorInitialized(address starkGateBridge, address tbtcToken);
}