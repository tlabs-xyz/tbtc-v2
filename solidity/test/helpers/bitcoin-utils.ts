...
import {
  ...
  ParsedOutput,
  TxInfo,
} from "../account-control/helpers/SPVTestData"
...
export class BitcoinUtils {
  ...
  static parseTransaction(txHex: string): ParsedTransaction {
    ...
    const { value: inputCount, length: inputCountLength } = this.parseVarInt(
      hex.substr(offset)
    )
    ...
    const { value: outputCount, length: outputCountLength } = this.parseVarInt(
      hex.substr(offset)
    )
   ...
   */
  static generateValidBitcoinAddress(
    type: "p2pkh" | "p2sh" | "bech32" = "p2pkh"
  ): string {
    switch (type) {
      case "p2pkh":
  ...
  static createRedemptionTransaction(
    ...
    const input = `01${
      // Input count: 1
      prevTxHash
    }${prevTxIndex}${scriptSig}${sequence}`
  ...
  static txHexToTxInfo(txHex: string): TxInfo {
    ...
    for (let i = 0; i < inputCount; i++) {
      ...
      const { value: scriptLength, length: scriptLengthBytes } =
        this.parseVarInt(hex.substr(offset))
  ...
  private static parseInput(hex: string): {
    input: ParsedInput
    length: number
  } {
    let offset = 0
    ...
    // Script sig length
    const { value: scriptLength, length: scriptLengthBytes } = this.parseVarInt(
      hex.substr(offset)
    )
  ...
  private static parseOutput(hex: string): {
    output: ParsedOutput
    length: number
  } {
    let offset = 0
    ...
    if (
      scriptLength === 25 &&
      hex.substr(offset - 50, 6) === "76a914" &&
      hex.substr(offset - 4, 4) === "88ac"
    ) {
