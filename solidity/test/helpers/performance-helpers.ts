...
export class PerformanceProfiler {
  ...
  assertGasWithinLimit(
    ...
    if (baseline) {
      ...
      expect(actualGas).to.be.lessThanOrEqual(
        maxAllowed,
        `Gas usage for ${operation} exceeded baseline by more than ${tolerance * 100}%`
      )
    }
  }
  ...
  getMetrics(): PerformanceMetrics {
    ...
    const gasProfiles = this.profiles.filter(p => !p.operation.includes("(time)"))
    const totalGas = gasProfiles.reduce((sum, p) => sum + p.gasUsed, 0)
    const maxGas = gasProfiles.reduce((max, p) => p.gasUsed > max.gasUsed ? p : max)
    const minGas = gasProfiles.reduce((min, p) => p.gasUsed < min.gasUsed ? p : min)

    return {
      totalGasUsed: totalGas,
      averageGasPerOperation: gasProfiles.length > 0 ? totalGas / gasProfiles.length : 0,
  ...
  generateReport(): string {
    ...
    report += `Average Gas per Operation: ${Math.round(metrics.averageGasPerOperation).toLocaleString()}\n`
    report += `Most Expensive: ${metrics.maxGasOperation.operation} (${metrics.maxGasOperation.gasUsed.toLocaleString()} gas)\n`
    report += `Least Expensive: ${metrics.minGasOperation.operation} (${metrics.minGasOperation.gasUsed.toLocaleString()} gas)\n`
