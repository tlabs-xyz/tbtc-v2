  ...
  for (let i = 0; i < Math.min(signers.length, 10); i++) {
    ...
    try {
      ...
      if (!error.message?.includes("AlreadyAuthorized")) {
        console.log(`⚠️ Warning: Could not authorize ${signer.address}:`, error.message);
      }
    }
  }
}
...
export async function setupQCMinterWithRoles(
  ...
  try {
    ...
    const hasGovernanceRole = await qcMinter.hasRole(GOVERNANCE_ROLE, owner.address);
    ...
    if (minters.length > 0) {
      ...
      for (const minter of minters) {
        const hasMinterRole = await qcMinter.hasRole(MINTER_ROLE, minter.address);
        if (!hasMinterRole) {
...
export async function setupOracleIntegration(
  ...
  try {
    ...
    const hasMinterRole = await accountControl.hasRole(MINTER_ROLE, qcManager.address);

    if (!hasMinterRole) {
      await accountControl.connect(owner).grantRole(MINTER_ROLE, qcManager.address);
      console.log("✓ QCManager granted MINTER_ROLE in AccountControl");
    }
  } catch (error) {
    console.log("⚠️ Warning: Could not setup Oracle integration:", error.message);
  }
}

/**
 * Setup SPV libraries with proper deployment and linking
 */
export async function setupSPVLibrariesForTesting() {
  try {
    // Deploy SharedSPVCore first
    ...
    // Deploy QCRedeemerSPV with SharedSPVCore dependency
    ...
    // Deploy TestRelay for SPV testing
    ...
    console.log("✓ SPV libraries deployed and linked");
    ...
    console.log("⚠️ Error setting up SPV libraries:", error.message);
    throw error;
  }
}
...
export async function setupAccountControlWithOwnership(
  ...
  try {
    ...
    console.log("⚠️ Warning: Could not setup AccountControl ownership:", error.message);
  }
}
 ...
 */
export async function setupSystemStateDefaults(systemState: any, deployer: SignerWithAddress) {
  try {
    ...
    if (!hasRole) {
      ...
      console.log("✓ OPERATIONS_ROLE granted to deployer for SystemState configuration");
    }

    // Set testing-friendly defaults for mint amounts (in wei/18 decimals for tBTC)
    await systemState.connect(deployer).setMinMintAmount(ethers.utils.parseEther("0.001")); // 0.001 tBTC
    await systemState.connect(deployer).setMaxMintAmount(ethers.utils.parseEther("1000"));  // 1000 tBTC
 ...
 */
export async function setupCompleteTestEnvironment(contracts: any, signers: SignerWithAddress[]) {
  ...
  try {
    ...
    if (contracts.accountControl) {
      await setupAccountControlForTesting(contracts.accountControl, signers, owner);
    }

    // Setup QCManager and QCData integration
    if (contracts.qcManager && contracts.qcData) {
      await setupQCManagerWithRoles(contracts.qcManager, contracts.qcData, owner);
    }

    // Setup QCMinter roles
    if (contracts.qcMinter) {
      await setupQCMinterWithRoles(contracts.qcMinter, owner, signers.slice(1, 4));
    }

    // Setup Oracle integration
    if (contracts.qcManager && contracts.accountControl && contracts.reserveOracle) {
    ...
    console.log("⚠️ Warning: Some test environment setup failed:", error.message);
  }
}

/**
 * Helper to get library linking configuration for QCRedeemer
 */
export function getQCRedeemerLibraries(libraries: { qcRedeemerSPV: any }) {
  return {
    libraries: {
 ...
 */
export function getBitcoinAddressUtilsLibraries(libraries: { bitcoinAddressUtils: any }) {
  return {
    libraries: {
