...
describe("WatchdogMonitor Emergency Detection Tests", () => {
  ...
  beforeEach(async () => {
    ...
    await qcWatchdog1.grantRole(
      await qcWatchdog1.WATCHDOG_OPERATOR_ROLE(),
      watchdog1.address
    )
    await qcWatchdog2.grantRole(
      await qcWatchdog2.WATCHDOG_OPERATOR_ROLE(),
      watchdog2.address
    )
    await qcWatchdog3.grantRole(
      await qcWatchdog3.WATCHDOG_OPERATOR_ROLE(),
      watchdog3.address
    )
    await qcWatchdog4.grantRole(
      await qcWatchdog4.WATCHDOG_OPERATOR_ROLE(),
      watchdog4.address
    )
    ...
    await watchdogMonitor
      .connect(governance)
      .registerWatchdog(qcWatchdog4.address, "Watchdog 4")

    // Set emergency council
    await systemState
      .connect(governance)
      .grantRole(await systemState.PAUSER_ROLE(), emergencyCouncil.address)
  })
  ...
  describe("Emergency Report Threshold (3 reports/hour)", () => {
    it("should trigger emergency pause when 3 reports received within 1 hour", async () => {
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, issue)
      )
      ...
      await watchdogMonitor
        .connect(watchdog2)
        .reportEmergency(qcWatchdog2.address, issue)
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog3)
          .reportEmergency(qcWatchdog3.address, issue)
      )
    ...
    it("should handle sliding window correctly", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "Issue 1")

      // Report 2 at t=30min
      await helpers.time.increaseTime(30 * MINUTE)
      await watchdogMonitor
        .connect(watchdog2)
        .reportEmergency(qcWatchdog2.address, "Issue 2")
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog3)
          .reportEmergency(qcWatchdog3.address, "Issue 3")
      )
        .to.emit(watchdogMonitor, "EmergencyTriggered")
        .withArgs(3)
    ...
    it("should count unique watchdog reports only", async () => {
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, issue + " duplicate")
  ...
  describe("Emergency Response Actions", () => {
    beforeEach(async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "Emergency test")
      await watchdogMonitor
        .connect(watchdog2)
        .reportEmergency(qcWatchdog2.address, "Emergency test")
      await watchdogMonitor
        .connect(watchdog3)
        .reportEmergency(qcWatchdog3.address, "Emergency test")
    })
  ...
  describe("Report Window Management", () => {
    it("should reset report count after window expires", async () => {
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, "New issue")
      ).to.not.be.reverted

      // Add two more reports - should trigger emergency
      await watchdogMonitor
        .connect(watchdog3)
        .reportEmergency(qcWatchdog3.address, "New issue")

      await expect(
        watchdogMonitor
          .connect(watchdog4)
          .reportEmergency(qcWatchdog4.address, "New issue")
      )
        .to.emit(watchdogMonitor, "EmergencyTriggered")
        .withArgs(3)
    })
    ...
    it("should handle reports at window boundaries correctly", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "Boundary test")

      // Report 2 at t=59min59s
      await helpers.time.increaseTime(HOUR - 1)
      await watchdogMonitor
        .connect(watchdog2)
        .reportEmergency(qcWatchdog2.address, "Boundary test")
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog3)
          .reportEmergency(qcWatchdog3.address, "Boundary test")
      )
        .to.emit(watchdogMonitor, "EmergencyTriggered")
        .withArgs(3)
    })
  ...
  describe("Integration with System Recovery", () => {
    it("should support graduated response based on report count", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "Minor issue")
      expect(await systemState.allPaused()).to.be.false

      // 2 reports - heightened monitoring (in real impl)
      await watchdogMonitor
        .connect(watchdog2)
        .reportEmergency(qcWatchdog2.address, "Growing concern")
      expect(await systemState.allPaused()).to.be.false

      // 3 reports - emergency pause
      await watchdogMonitor
        .connect(watchdog3)
        .reportEmergency(qcWatchdog3.address, "Critical issue")
      expect(await systemState.allPaused()).to.be.true
    })
    ...
    it("should emit detailed events for monitoring", async () => {
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, "Detailed issue description")
      )
        .to.emit(watchdogMonitor, "EmergencyReported")
        .withArgs(
          qcWatchdog1.address,
  ...
  describe("Access Control and Security", () => {
    it("should only allow registered watchdogs to report", async () => {
      const unregisteredWatchdog = await helpers.signers
        .getUnnamedSigners()
        .then((s) => s[10])

      await expect(
        watchdogMonitor
          .connect(unregisteredWatchdog)
          .reportEmergency(qcWatchdog1.address, "Unauthorized report")
    ...
    it("should prevent report spam from compromised watchdog", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "First report")

      // Immediate second report from same watchdog fails
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, "Spam report")
      ).to.be.revertedWith("Watchdog already reported in current window")
      ...
      await expect(
        watchdogMonitor
          .connect(watchdog2)
          .reportEmergency(qcWatchdog2.address, "Legitimate report")
      ).to.not.be.reverted
    })
    ...
    it("should handle watchdog removal gracefully", async () => {
      ...
      await watchdogMonitor
        .connect(watchdog1)
        .reportEmergency(qcWatchdog1.address, "Issue before removal")

      // Remove watchdog1
      await watchdogMonitor
        .connect(governance)
        .removeWatchdog(qcWatchdog1.address)

      // Watchdog1 can no longer report
      await expect(
        watchdogMonitor
          .connect(watchdog1)
          .reportEmergency(qcWatchdog1.address, "Post-removal report")
