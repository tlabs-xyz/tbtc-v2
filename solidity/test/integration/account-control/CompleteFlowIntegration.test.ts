...
class CompleteFlowIntegration extends BaseAccountControlIntegration {
  ...
  async runPolicyUpgradeTest(): Promise<void> {
    ...
    await this.qcMinter.connect(this.user).requestQCMint(this.qc.address, mintAmount)
    ...
    const newMintingPolicy = await NewMintingPolicy.deploy(this.protocolRegistry.address)
    ...
    await this.qcMinter.connect(this.user).requestQCMint(this.qc.address, newMintAmount)
    ...
    if (oldMinAmount.lt(ethers.utils.parseEther("0.5"))) {
      ...
      await expect(
        this.qcMinter.connect(this.user).requestQCMint(this.qc.address, oldMinAmount)
  ...
  async runEmergencyScenarios(): Promise<void> {
    ...
    await expect(
      this.qcMinter.connect(this.user).requestQCMint(this.qc.address, ethers.utils.parseEther("1"))
  ...
  async runStressTest(): Promise<void> {
    ...
    for (let i = 3; i < 10; i++) { // Use signers 3-9 (skip deployer, governance, qc)
      mintPromises.push(
        this.qcMinter.connect(users[i]).requestQCMint(this.qc.address, mintAmount)
      )
    }
    ...
    console.log(`✅ High volume minting: ${successful} successful, ${failed} failed`)
    ...
    console.log(`Available capacity: ${ethers.utils.formatEther(availableCapacity)} tBTC`)

    if (availableCapacity.gt(0)) {
      // Try to mint more than available
      const excessiveAmount = availableCapacity.add(ethers.utils.parseEther("1"))
      
      await expect(
        this.qcMinter.connect(users[10]).requestQCMint(this.qc.address, excessiveAmount)
  ...
  private async runUserRedemption(): Promise<void> {
    ...
    // Fulfill redemption
    const fulfillmentProof = this.generateFulfillmentProof(redemptionId, btcAddress, redeemAmount)
  ...
  private async runSystemVerification(): Promise<void> {
    ...
    expect(currentTime - lastAttestationTime).to.be.lte(this.TEST_PARAMS.ATTESTATION_STALE_THRESHOLD)
