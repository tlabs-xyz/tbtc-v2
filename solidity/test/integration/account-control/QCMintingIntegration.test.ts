...
class QCMintingIntegration extends BaseAccountControlIntegration {
  async runTest(): Promise<void> {
    ...
    const mintRequestEvent = mintReceipt.events?.find(e => e.event === "QCMintRequested")
    ...
    const balanceIncreasedEvent = mintReceipt.events?.find(e => e.event === "BalanceIncreased")
  ...
  async runConcurrentMintingTest(): Promise<void> {
    ...
    for (let i = 0; i < mintReceipts.length; i++) {
      ...
      const mintEvent = receipt.events?.find(e => e.event === "QCMintRequested")
  ...
  async runEdgeCasesTest(): Promise<void> {
    ...
    const nearCapacityAmount = availableCapacity.sub(ethers.utils.parseEther("1"))
    
    await expect(
      this.qcMinter.connect(this.user).requestQCMint(this.qc.address, nearCapacityAmount)
    ).to.not.be.reverted
    ...
    const remainingCapacity = await this.qcManager.getAvailableMintingCapacity(this.qc.address)
    
    if (remainingCapacity.gt(0)) {
      await expect(
        this.qcMinter.connect(this.user).requestQCMint(this.qc.address, remainingCapacity)
      ).to.not.be.reverted
    }
  ...
  async runErrorScenarios(): Promise<void> {
    ...
    await expect(
      this.qcMinter.connect(this.user).requestQCMint(this.qc.address, tooSmallAmount)
    ...
    await expect(
      this.qcMinter.connect(this.user).requestQCMint(this.qc.address, tooLargeAmount)
    ...
    await expect(
      this.qcMinter.connect(this.user).requestQCMint(this.qc.address, excessiveAmount)
