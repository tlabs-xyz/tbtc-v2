...
class ReserveAttestationIntegration extends BaseAccountControlIntegration {
  async runTest(): Promise<void> {
    ...
    console.log(
      `✅ Monitored ${
        qcWallets.length
      } wallets, found ${ethers.utils.formatEther(totalReserves)} BTC`
    )
    ...
    const attestationEvent = attestationReceipt.events?.find(
      (e) => e.event === "ReserveAttestationSubmitted"
    )
    ...
    const solvencyRatio = totalReserves
      .mul(100)
      .div(mintedAmount.add(ethers.utils.parseEther("1"))) // Avoid division by zero

    expect(solvencyRatio).to.be.greaterThan(100) // Should be over-collateralized
    console.log(
      `✅ QC is solvent with ${solvencyRatio.toString()}% collateralization`
    )
    ...
    console.log(
      `✅ Available minting capacity: ${ethers.utils.formatEther(
        availableCapacity
      )} tBTC`
    )

    console.log(
      "🎉 Reserve Attestation Integration Test completed successfully"
    )
  }
  ...
  async runUndercollateralizedTest(): Promise<void> {
    ...
    await this.qcManager
      ...
      .updateQCMintedAmount(this.qc.address, mintedAmount)
    ...
    const statusChangeEvent = attestationReceipt.events?.find(
