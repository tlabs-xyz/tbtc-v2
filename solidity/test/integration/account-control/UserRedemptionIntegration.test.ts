...
class UserRedemptionIntegration extends BaseAccountControlIntegration {
  async runTest(): Promise<void> {
    ...
    console.log(`✅ User has ${ethers.utils.formatEther(userInitialBalance)} tBTC`)
    ...
    const redemptionEvent = redeemReceipt.events?.find(e => e.event === "RedemptionRequested")
    ...
    // Verify burn event
    const burnEvent = redeemReceipt.events?.find(e => e.event === "Transfer" && e.args?.to === ethers.constants.AddressZero)
    ...
    const fulfillmentProof = this.generateFulfillmentProof(redemptionId, btcReceiveAddress, redeemAmount)
    ...
    const fulfillmentEvent = fulfillmentReceipt.events?.find(e => e.event === "RedemptionFulfilled")
  ...
  async runTimeoutScenario(): Promise<void> {
    ...
    const defaultEvent = defaultReceipt.events?.find(e => e.event === "RedemptionDefaulted")
  ...
  async runConcurrentRedemptionTest(): Promise<void> {
    ...
    const redeemPromises = [
      this.qcRedeemer.connect(user1).initiateRedemption(this.qc.address, redeemAmount, btcAddresses[0]),
      this.qcRedeemer.connect(user2).initiateRedemption(this.qc.address, redeemAmount, btcAddresses[1]),
      this.qcRedeemer.connect(user3).initiateRedemption(this.qc.address, redeemAmount, btcAddresses[2])
    ]
    ...
    for (let i = 0; i < redeemReceipts.length; i++) {
      ...
      const redemptionEvent = receipt.events?.find(e => e.event === "RedemptionRequested")
      expect(redemptionEvent).to.exist
    ...
    for (let i = 0; i < redemptionIds.length; i++) {
      ...
      const fulfillmentProof = this.generateFulfillmentProof(redemptionId, btcAddresses[i], redeemAmount)
  ...
  async runErrorScenarios(): Promise<void> {
    ...
    const redeemTx = await this.qcRedeemer.connect(this.user).initiateRedemption(
      this.qc.address,
      ethers.utils.parseEther("1"),
      "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4"
    )
  ...
  private async setupUserWithTokens(): Promise<void> {
    ...
    await this.qcManager.connect(this.deployer).updateQCMintedAmount(this.qc.address, userTokens)
  }

  private generateFulfillmentProof(redemptionId: string, btcAddress: string, amount: any): any {
    return {
