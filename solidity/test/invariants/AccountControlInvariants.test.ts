...
describe("AccountControl Invariant Tests", () => {
  ...
  describe("Invariant 1: Backing >= Minted", () => {
    it("should maintain backing >= minted after single mint", async () => {
      await accountControl
        .connect(reserve1)
        .mintTBTC(user1.address, ethers.utils.parseEther("100"))
    ...
    it("should maintain backing >= minted after multiple mints", async () => {
      ...
      await accountControl
        .connect(reserve2)
        .mintTBTC(user2.address, ethers.utils.parseEther("200"))
      await accountControl
        .connect(reserve1)
        .mintTBTC(user1.address, ethers.utils.parseEther("50"))
      await checkAllInvariants()
    })
    ...
    it("should maintain backing >= minted after redemptions", async () => {
      ...
      await accountControl
        .connect(reserve1)
        .redeemTBTC(ethers.utils.parseEther("50"))
      await accountControl
        .connect(reserve2)
        .redeemTBTC(ethers.utils.parseEther("100"))
  ...
  describe("Invariant 2: Total Consistency", () => {
    it("should maintain total = sum consistency after complex operations", async () => {
      ...
      await accountControl
        .connect(reserve3)
        .mintTBTC(user1.address, ethers.utils.parseEther("150"))

      await accountControl
        .connect(reserve1)
        .redeemTBTC(ethers.utils.parseEther("30"))
      await accountControl
        .connect(reserve2)
        .redeemTBTC(ethers.utils.parseEther("50"))
      ...
      await accountControl
        .connect(reserve1)
        .mintTBTC(user2.address, ethers.utils.parseEther("75"))
    ...
    it("should maintain consistency after backing changes", async () => {
      ...
      await accountControl
        .connect(reserve1)
        .mintTBTC(user1.address, ethers.utils.parseEther("200"))
    ...
    it("should maintain all invariants under high-frequency operations", async () => {
      const operations = [
        ...
        () =>
          accountControl
            .connect(reserve3)
            .mintTBTC(user1.address, ethers.utils.parseEther("25")),
        () =>
          accountControl
            .connect(reserve1)
            .redeemTBTC(ethers.utils.parseEther("20")),
        () =>
          accountControl
            .connect(reserve2)
            .redeemTBTC(ethers.utils.parseEther("30")),
        () =>
          accountControl
            .connect(reserve1)
            .mintTBTC(user2.address, ethers.utils.parseEther("40")),
        () =>
          accountControl
            .connect(reserve3)
            .redeemTBTC(ethers.utils.parseEther("15")),
        () =>
          accountControl
            .connect(reserve2)
            .mintTBTC(user1.address, ethers.utils.parseEther("60")),
      ]
