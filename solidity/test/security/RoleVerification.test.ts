...
describe("Role-Based Access Control Security Tests", () => {
  ...
  const REDEEMER_ROLE = ethers.utils.keccak256(
  ...
  const QC_MANAGER_ROLE = ethers.utils.keccak256(
    ethers.utils.toUtf8Bytes("QC_MANAGER_ROLE")
  )
  const QC_GOVERNANCE_ROLE = ethers.utils.keccak256(
    ethers.utils.toUtf8Bytes("QC_GOVERNANCE_ROLE")
  )

  async function fixture() {
    const signers = await ethers.getSigners()
    ;[deployer, governance, attacker, operator1, operator2, randomUser] =
      signers
  ...
  describe("SystemState Access Control", () => {
    it("should deploy with deployer as DEFAULT_ADMIN_ROLE", async () => {
      expect(await systemState.hasRole(DEFAULT_ADMIN_ROLE, deployer.address)).to
        .be.true
      expect(await systemState.getRoleAdmin(PARAMETER_ADMIN_ROLE)).to.equal(
        DEFAULT_ADMIN_ROLE
      )
      expect(await systemState.getRoleAdmin(PAUSER_ROLE)).to.equal(
        DEFAULT_ADMIN_ROLE
      )
    })
    ...
    it("should prevent non-admin from granting roles", async () => {
      await expect(
        systemState
          .connect(attacker)
          .grantRole(PARAMETER_ADMIN_ROLE, attacker.address)
      ).to.be.revertedWith(
    ...
    it("should allow PARAMETER_ADMIN to update parameters", async () => {
      ...
      await expect(
        systemState.connect(governance).setMinMintAmount(newMinAmount)
      )
    ...
    it("should enforce proper role hierarchy", async () => {
      ...
      await expect(
        systemState
          .connect(governance)
          .grantRole(PAUSER_ROLE, operator1.address)
  ...
  describe("QCManager Access Control", () => {
    beforeEach(async () => {
      ...
      await protocolRegistry.setService(
        ethers.utils.id("QC_RESERVE_LEDGER"),
    ...
    it("should prevent non-QC_ADMIN from registering QCs", async () => {
      await expect(
        qcManager
          .connect(attacker)
          .registerQC(randomUser.address, ethers.utils.parseEther("1000"))
      ).to.be.revertedWith(
  ...
  describe("Cross-Contract Role Dependencies", () => {
    it("should verify WatchdogConsensusManager has ARBITER_ROLE on QCManager", async () => {
      ...
      expect(
        await qcManager.hasRole(ARBITER_ROLE, watchdogConsensusManager.address)
      ).to.be.false
    ...
    it("should verify BasicMintingPolicy has QC_ADMIN_ROLE on QCManager", async () => {
      ...
      expect(await qcManager.hasRole(QC_ADMIN_ROLE, basicMintingPolicy.address))
        .to.be.true
    })
    ...
    it("should verify QCMinter has MINTER_ROLE on BasicMintingPolicy", async () => {
      ...
      expect(await basicMintingPolicy.hasRole(MINTER_ROLE, qcMinter.address)).to
        .be.true
    })
  })
  ...
  describe("Role Transfer and Revocation", () => {
    it("should properly transfer admin role", async () => {
      ...
      expect(await systemState.hasRole(DEFAULT_ADMIN_ROLE, governance.address))
        .to.be.true

      // Governance can now grant roles
      await systemState
        .connect(governance)
        .grantRole(PARAMETER_ADMIN_ROLE, operator1.address)
      expect(await systemState.hasRole(PARAMETER_ADMIN_ROLE, operator1.address))
        .to.be.true
    })
    ...
    it("should properly revoke roles", async () => {
      ...
      expect(await systemState.hasRole(PAUSER_ROLE, operator1.address)).to.be
        .true
    ...
    it("should allow role renunciation", async () => {
      ...
      await systemState
        .connect(operator1)
        .renounceRole(PAUSER_ROLE, operator1.address)
      expect(await systemState.hasRole(PAUSER_ROLE, operator1.address)).to.be
        .false
    })
  ...
  describe("Zero Address and Edge Cases", () => {
    ...
    it("should handle multiple roles on same account", async () => {
      ...
      await systemState
        .connect(operator1)
        .setMinMintAmount(ethers.utils.parseEther("50"))
    ...
    it("should maintain role separation between contracts", async () => {
      ...
      expect(
        await watchdogConsensusManager.hasRole(MANAGER_ROLE, operator1.address)
      ).to.be.false
  ...
  describe("Emergency Scenarios", () => {
    it("should handle admin loss prevention", async () => {
      ...
      await systemState.grantRole(DEFAULT_ADMIN_ROLE, governance.address)
      expect(await systemState.getRoleMemberCount(DEFAULT_ADMIN_ROLE)).to.equal(
        2
      )
