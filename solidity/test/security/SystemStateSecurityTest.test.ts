...
describe("SystemState Security Tests", () => {
  ...
  async function fixture() {
    ...
    ;[deployer, governance, pauser, paramAdmin, attacker, emergencyCouncil] = signers
  ...
  describe("Pause Mechanism Security", () => {
    describe("Access Control", () => {
      it("should only allow PAUSER_ROLE to pause operations", async () => {
        ...
        await expect(systemState.connect(attacker).pauseMinting())
          .to.be.revertedWith(`AccessControl: account ${attacker.address.toLowerCase()} is missing role ${PAUSER_ROLE}`)
      ...
      it("should only allow PAUSER_ROLE to unpause operations", async () => {
        ...
        await expect(systemState.connect(attacker).unpauseMinting())
          .to.be.revertedWith(`AccessControl: account ${attacker.address.toLowerCase()} is missing role ${PAUSER_ROLE}`)
    ...
    describe("Pause State Management", () => {
      ...
      it("should track pause timestamps", async () => {
        ...
        const mintingPauseKey = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("MINTING_PAUSE"))
        expect(await systemState.pauseTimestamps(mintingPauseKey)).to.equal(pauseTime)
      })
    ...
    describe("Emergency Pause Duration", () => {
      it("should enforce emergency pause duration limits", async () => {
        ...
        await expect(systemState.connect(paramAdmin).setEmergencyPauseDuration(maxDuration + 1))
      ...
      it("should track pause duration for monitoring", async () => {
        ...
        const mintingPauseKey = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("MINTING_PAUSE"))
        const pauseTimestamp = await systemState.pauseTimestamps(mintingPauseKey)
    ...
    describe("Concurrent Pause Attempts", () => {
      it("should handle concurrent pause attempts safely", async () => {
        ...
        await systemState.connect(governance).grantRole(PAUSER_ROLE, emergencyCouncil.address)

        // First pause succeeds
        await systemState.connect(pauser).pauseMinting()

        // Second pause attempt fails
        await expect(systemState.connect(emergencyCouncil).pauseMinting())
          .to.be.revertedWith("MintingAlreadyPaused")
      })
    })
  })
  ...
  describe("Parameter Validation", () => {
    describe("Mint Amount Parameters", () => {
      it("should validate min/max relationship", async () => {
        await systemState.connect(paramAdmin).setMaxMintAmount(ethers.utils.parseEther("1000"))
        
        // Cannot set min > max
        await expect(
          systemState.connect(paramAdmin).setMinMintAmount(ethers.utils.parseEther("2000"))
        ).to.be.revertedWith("MinAmountExceedsMax")
      })

      it("should validate max >= min when setting max", async () => {
        await systemState.connect(paramAdmin).setMinMintAmount(ethers.utils.parseEther("100"))
        
        // Cannot set max < min
        await expect(
          systemState.connect(paramAdmin).setMaxMintAmount(ethers.utils.parseEther("50"))
    ...
    describe("Collateral Ratio", () => {
      it("should enforce collateral ratio bounds", async () => {
        // Cannot set below 50%
        await expect(
          systemState.connect(paramAdmin).setMinCollateralRatio(49)
        ).to.be.revertedWith("InvalidAmount")
  ...
  describe("Emergency Council", () => {
    it("should only allow PARAMETER_ADMIN to set emergency council", async () => {
      await expect(
        systemState.connect(attacker).setEmergencyCouncil(emergencyCouncil.address)
      ).to.be.revertedWith(`AccessControl: account ${attacker.address.toLowerCase()} is missing role ${PARAMETER_ADMIN_ROLE}`)
    })

    it("should prevent setting zero address as emergency council", async () => {
      await expect(
        systemState.connect(paramAdmin).setEmergencyCouncil(ethers.constants.AddressZero)
      ).to.be.revertedWith("InvalidCouncilAddress")
    })

    it("should emit event when setting emergency council", async () => {
      await expect(
        systemState.connect(paramAdmin).setEmergencyCouncil(emergencyCouncil.address)
      ).to.emit(systemState, "EmergencyCouncilUpdated")
        .withArgs(ethers.constants.AddressZero, emergencyCouncil.address, paramAdmin.address)
    })
  })
  ...
  describe("Parameter Update Edge Cases", () => {
    it("should handle parameter updates at boundaries", async () => {
      ...
      await systemState.connect(paramAdmin).setMinCollateralRatio(50)
      expect(await systemState.minCollateralRatio()).to.equal(50)
    ...
    it("should maintain parameter consistency across updates", async () => {
      ...
      await systemState.connect(paramAdmin).setMinMintAmount(ethers.utils.parseEther("10"))
      await systemState.connect(paramAdmin).setMaxMintAmount(ethers.utils.parseEther("1000"))
      
      // Update min - should maintain relationship
      await systemState.connect(paramAdmin).setMinMintAmount(ethers.utils.parseEther("100"))
      expect(await systemState.minMintAmount()).to.equal(ethers.utils.parseEther("100"))
      expect(await systemState.maxMintAmount()).to.equal(ethers.utils.parseEther("1000"))
    })
  })
  ...
  describe("View Function Security", () => {
    ...
    it("should revert operations when paused", async () => {
      ...
      await expect(
        systemState.requireMintingNotPaused()
      ).to.be.revertedWith("MintingIsPaused")
    })
    ...
    it("should expose all parameters correctly", async () => {
      ...
      await systemState.connect(paramAdmin).setMinCollateralRatio(90)

      // Verify all are readable
      expect(await systemState.minMintAmount()).to.equal(ethers.utils.parseEther("10"))
      expect(await systemState.redemptionTimeout()).to.equal(48 * 60 * 60)
      expect(await systemState.minCollateralRatio()).to.equal(90)
    })
  })

  describe("Initialization Security", () => {
    it("should initialize with secure defaults", async () => {
      const freshSystemState = await (await ethers.getContractFactory("SystemState")).deploy()

      // Check default values are sensible
      expect(await freshSystemState.minMintAmount()).to.equal(ethers.utils.parseEther("0.01"))
      expect(await freshSystemState.maxMintAmount()).to.equal(ethers.utils.parseEther("21000000"))
