...
import { StarkNetTBTCTokenConfig } from "../../src/lib/starknet"
...
describe("StarkNet Single-Parameter Deposit Flow", () => {
  ...
  beforeEach(async () => {
    ...
    sinon.stub(starknet, "loadStarkNetCrossChainContracts").callsFake(async (walletAddress: string, provider?: any, chainId: string = "0x534e5f5345504f4c4941") => {
      const depositorInterface = new starknet.StarkNetDepositorInterface()
      depositorInterface.setDepositOwner(starknet.StarkNetAddress.from(walletAddress))
      ...
      return {
        ...
        l2TbtcToken: new starknet.StarkNetTBTCToken(config, mockProvider as any),
      }
    })
    ...
    const crossChainContractsLoader = {
      ...
      loadL1Contracts: async (l2ChainName: string) => ({
        l1BitcoinDepositor: {
          getChainIdentifier: () => ({ identifierHex: "0x1234567890abcdef1234567890abcdef12345678" }),
        },
      }),
    ...
    tbtc = new (TBTC as any)(mockContracts, mockBitcoinClient, crossChainContractsLoader)
  ...
  describe("Complete deposit flow with single-parameter", () => {
    it("should complete deposit with only StarkNet wallet", async () => {
      // Arrange
      const starknetAddress = "0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7"
      ...
      axiosStub.resolves({
        data: { 
          receipt: { 
            transactionHash: "0xabc123def456789abc123def456789abc123def456789abc123def456789ab" 
          } 
        }
      })
      ...
      const deposit: DepositReceipt = {
        ...
        extraData: Hex.from("049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7"),
      }
      ...
      // Assert
      expect(result.toString()).to.equal("abc123def456789abc123def456789abc123def456789abc123def456789ab")
      expect(axiosStub.calledOnce).to.be.true
      expect(axiosStub.args[0][0]).to.equal("http://relayer.tbtcscan.com/api/reveal")
    ...
    it("should preserve StarkNet address through entire flow", async () => {
      ...
      axiosStub.callsFake(async (url: string, data: any) => {
        ...
        return {
          data: { 
            receipt: { 
              transactionHash: "0xdef456789abc123def456789abc123def456789abc123def456789abc123de" 
            } 
          }
        }
      })
    ...
    it("should handle relayer failures gracefully", async function () {
      ...
      const setTimeoutStub = sinon.stub(global, "setTimeout").callsFake((fn: any) => {
      ...
      await expect(
        depositor.initializeDeposit(
          ...
          0,
          {
            depositor: StarkNetAddress.from("0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7"),
            walletPublicKeyHash: Hex.from("1234567890abcdef1234567890abcdef12345678"),
            refundPublicKeyHash: Hex.from("abcdef1234567890abcdef1234567890abcdef12"),
            blindingFactor: Hex.from("f9f0c90d00039523"),
            refundLocktime: Hex.from("60920000"),
  ...
  describe("Backward compatibility", () => {
    it("should show deprecation warning for two-parameter mode", async () => {
      ...
      // Act
      await tbtc.initializeCrossChain("StarkNet", ethereumSigner as any, starknetProvider)
