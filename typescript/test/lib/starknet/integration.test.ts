...
import { EthereumL1BitcoinDepositor } from "../../../src/lib/ethereum"
...
describe("StarkNet Integration Tests", () => {
  describe("L1 Bitcoin Depositor with StarkNet Integration", () => {
    it("should encode StarkNet recipient address for L1 deposit", async () => {
      ...
      const starkNetRecipient = "0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7"
      ...
      expect(extraData.toPrefixedString()).to.equal(recipientAddress.toBytes32())
    })
    ...
    it("should handle full deposit flow with StarkNet", async () => {
      ...
      expect(decodedAddress.identifierHex).to.equal(recipientAddress.identifierHex)
    })
  ...
  describe("Cross-Component Integration", () => {
    ...
    it("should validate addresses consistently across components", async () => {
      ...
      for (const invalid of invalidAddresses) {
        ...
        await expect(
          loadStarkNetCrossChainContracts(invalid)
        ).to.be.rejected
      }
    })
